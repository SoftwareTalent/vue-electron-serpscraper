...
ipcMain.on('pause-scrape', async (event, args) => {
  console.log('hello pause', args)
  
  PAUSE_SCRAPE = true
})

ipcMain.on('resume-scrape', async (event, args) => {
  console.log('hello resume', args)
...
/**
  ...
  */
export const search = async (params, callback) => {
  if (callback) {
    EVENT_FN = callback
  }
...
async function doRequest(options, nbrOfLinks = 0) {
  ...
  for (let i = 0; i < options.retry; i += 1) {
    try {
      ...
      let delayNumber = options.delay

      if (options.delay === 'random') {
        delayNumber = randomInteger(5000, 10000)
      }
...
function extractLinks(body) {
  ...
  // writeFileSync('./dump.txt', $('body').html())
  ...
  $('body').find('h3').each((i, h3) => {
    if ($(h3).parent()) {
      let href = $(h3).parent().attr('href');

      if (href) {
        href = href.replace('/url?q=', '').replace('/url?esrc=s&q=&rct=j&sa=U&url=', '').replace(/[&](.*)*/gi, '')
        const result = { url: href , title: $(h3).text() }
        links.push(result);
        if (EVENT_FN) {
          EVENT_FN(result)
        }
  ...
  const nextPage = $('.nBDE1b.G5eFlf').attr('href');
